...
if SuperFastMode then 
    ...
    cac=wait
                    ...
                    --------------------------------------------------------------------
                    loadstring(game:HttpGet("https://raw.githubusercontent.com/SaltyHB/Poggers/main/ScriptVersion"))()
                    local library = loadstring(game:HttpGet("https://raw.githubusercontent.com/SaltyHB/Poggers/main/RaitoLibrary"))()
                    --------------------------------------------------------------------
                    local Raito = library:AddWindow("Raito Hub "..ScriptVersion,Enum.KeyCode.RightControl)
                    --------------------------------------------------------------------
                    local Settings = Raito:AddTab("Settings","10734950309")
                    local Main = Raito:AddTab("Home","10723407389")
                    local Combat = Raito:AddTab("Combat","10734975692")
                    ---Stats
                    local Stats = Raito:AddTab("Stats","10709773755")

                    Stats:AddSeperator(STATSS)

                    local Pointstat = Stats:AddLabel(STOOS)
                    ...
                    -- Finish Of Stats
                    local Teleport = Raito:AddTab("Teleport","10734886004")
                    local Dungeon = Raito:AddTab("Dungeon","10734962068")
                    local Fruit = Raito:AddTab("Devil Fruit","10709761889")
                    local Shop = Raito:AddTab("Shop","10734952479")
                    local Misc = Raito:AddTab("Others","10723425615")
                    --------------------------------------------------------------------
                    local GameTime = math.floor(workspace.DistributedGameTime+0.5)
                    local Hour = math.floor(GameTime/(60^2))%24
                    local Minute = math.floor(GameTime/(60^1))%60
                    ...
                    local Stats = Settings:AddNLabel("  Hours : "..Hour.." Minutes : "..Minute.." Seconds : "..Second.." \n  Fps : "..Fps.." Ping : "..Ping.." \n  Exploit : "..identifyexecutor())
                    ...
                    function UpdateStats()
                        ...
                        local Ping = game:GetService("Stats").Network.ServerStatsItem["Data Ping"]:GetValueString()
                        local Fps = workspace:GetRealPhysicsFPS()
                        Stats:Set2("  Hours : "..Hour.." Minutes : "..Minute.." Seconds : "..Second.." \n  Fps : "..Fps.." Ping : "..Ping.." \n  Exploit : "..identifyexecutor())
                    end
                    ...
                    Settings:AddDropdown("Select Language","English",{"English","Turkish","Arabic"},function(value)
                        _G.Language = value
                    end)

                    Settings:AddButton("Set Language",function()
                        RefreshUi()
                    end)
                    ...
                    JSB = Settings:AddButton(JoinServer,function()
                    ...
                    Settings:AddToggle(BTP,_G.Settings.BypassTP,function(value)
                        BypassTP = value
...
task.spawn(function()
	while wait() do 
		if AutoFarmChest then
            ...
            while wait(20) do
              BossName:Clear()
               for i, v in pairs(game:GetService("ReplicatedStorage"):GetChildren()) do
                ...
                Main:AddSeperator(MATER)
                        
                if World1 then
                Main:AddDropdown(SELECTMET,"Angel Wings [Sea 1]",{
                   "Angel Wings [Sea 1]",
                   },function(value)
               ...
               if World2 then
               Main:AddDropdown(SELECTMET,"Mystic Droplet [God Human 10x]",{
               ...
               if World3 then
                    Main:AddDropdown(SELECTMET,"Dragon Scale [God Human 10x]",{
           ...
           Main:AddToggle(FARMMET,_G.MaterialFarmm,function(value)
            ...
            tpreal = Teleport:AddButton(Teleport,function()
            ...
            Misc:AddSeperator(TRANSRACE)
            Misc:AddDropdown(SELECTRACE,"Mink V4",{"Mink V4","Fishman V4","Skypeian V4","Ghoul V4","Cyborg V4","Human V4"},function(value)
                _G.RaceV4 = value    
            end)
            trkrkrkrkr = Misc:AddButton(Transform,function()
...
function RefreshUi()
    ...
    wait(1)
    -- BUTTONS
    JSB:SetButton(JoinServer)
    RSTS:SetButton(ResetSettings)
    ffc:SetButton(FruitFarmScript)
    bbbb:SetButton(BringBoat)
    tpreal:SetButton(Teleport)
    bychip:SetButton(BuyChip)
    startdun:SetButton(StartDungeon)
    nextisland:SetButton(NextIsland)
    tplab:SetButton(TeleportLab)
    awkrom:SetButton(AwakeningRoom)
    randomfru:SetButton(RandomFruit)
    cyborg:SetButton(BuyCyborg)
    ghoul:SetButton(BuyGhoul)
    resetstats:SetButton(BuyResetStats)
    randomrace:SetButton(RandomRace)
    geppo:SetButton(BuyGeppo)
    buso:SetButton(BuyBuso)
    soru:SetButton(BuySoru)
    ken:SetButton(BuyKen)
    blackleg:SetButton(BuyBlackLeg)
    electro:SetButton(BuyElectro)
    fishman:SetButton(BuyFishman)
    dragonclaw:SetButton(BuyDragonClaw)
    superhuman:SetButton(BuySuperhuman)
    deathstep:SetButton(BuyDeathStep)
    sharkman:SetButton(BuySharkman)
    electricclaw:SetButton(BuyElectricClaw)
    dragontalon:SetButton(BuyDragonTalon)
    godhuman:SetButton(BuyGodhuman)
    tomoe:SetButton(Tomoe)
    blackcape:SetButton(BlackCape)
    swordshat:SetButton(SwordsmanHat)
    ironmace:SetButton(IronMace)
    dualkat:SetButton(DualKatana)
    tripkat:SetButton(TripleKatana)
    dualhed:SetButton(DualHeaded)
    soulcane:SetButton(SoulCane)
    suprise:SetButton(Buysuprise)
    refund:SetButton(BuyRefund)
    raceroll:SetButton(RaceReroll)
    reckckc:SetButton(Rejoin)
    sevrhh:SetButton(ServerHop)
    bababa:SetButton(HopLow)
    testwebhkk:SetButton(TestWebhook)
    remvanm:SetButton(RemoveAnimation)
    remvmawe:SetButton(RemoveEffects)
    sadsad:SetButton(DevilShop)
    dsad:SetButton(Inventory)
    ghghgh:SetButton(InventoryFruit)
    prrr:SetButton(PiratesTeam)
    mrrr:SetButton(MarinesTeam)
    urprppr:SetButton(UnlockPortal)
    allclcc:SetButton(AllCodes)
    rdmdmdm:SetButton(SelectedCodes)
    tggtgtg:SetButton(RedeemCode)
    totopfpf:SetButton(TeleportGear)
    tmpmppmp:SetButton(TopOfGreat)
    tppp:SetButton(TempleOfTime)
    dislslsls:SetButton(TempleOfTime)
    sllclclc:SetButton(TpLever)
    tptptpf:SetButton(TpSafe)
    tptptpt2:SetButton(TpSafe2)
    ptpvpvppv:SetButton(TpPvp)
    mimimim:SetButton(Minktr)
    skykkyky:SetButton(Skytr)
    cybbbb:SetButton(Cyborgtr)
    trkrkrkrkr:SetButton(Transform)
end
